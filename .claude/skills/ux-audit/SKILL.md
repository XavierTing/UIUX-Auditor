...
- **Method 2 — Figma REST API via `figma_export.py`:** Use the Figma REST API to automatically discover and download all frames as PNG:
  ```bash
  python3 "<project-root>/.claude/skills/ux-audit/scripts/figma_export.py" \
...
```bash
cd "<project-root>/.claude/skills/ux-audit/scripts" && npm install playwright 2>/dev/null && npx playwright install chromium 2>/dev/null
```

Capture all screens:
```bash
node "<project-root>/.claude/skills/ux-audit/scripts/capture.js" <base-url> "<output-dir>/screenshots" --routes /,/route1,/route2 [--password pwd]
```
...
```bash
python3 "<project-root>/.claude/skills/ux-audit/scripts/annotate.py" \
...
SCRIPT = '<project-root>/.claude/skills/ux-audit/scripts/annotate.py'
...
```bash
python3 "<project-root>/.claude/skills/ux-audit/scripts/generate_html.py" \
  "<output-dir>" "<output-dir>/report.html"
```

> **Note — dated subfolder per regeneration.** `generate_html.py` routes the HTML output into a fresh per-generation subfolder under the audit directory: `<audit-dir>/reports/<YYYY-MM-DD>/report.html`. Each regeneration creates a new dated folder; the canonical audit assets (`report.md`, `findings.json`, `screenshots/`) stay in the audit root and are not duplicated. The `<output-file>` argument's basename is preserved (default `report.html`); its directory portion is ignored.

> **Note — visual style.** Reports render in an Anthropic-Research-inspired light aesthetic: cream `#FAF9F5` background, serif display headings (Source Serif 4), Inter body, mono numerics (JetBrains Mono), hairline dividers in place of card shadows, and an earthy severity palette (terracotta / ochre / olive / moss). Screenshots are wired side-by-side with their finding cards via dotted SVG connector lines that terminate at each marker's pill edge. PIL markers themselves are pill-shaped (matching the in-page badge), bold Menlo Bold digits.

**PowerPoint report** (slide deck with embedded screenshots):
```bash
python3 "<project-root>/.claude/skills/ux-audit/scripts/generate_pptx.py" \
...
```bash
python3 "<project-root>/.claude/skills/ux-audit/scripts/generate_combined_html.py" \
  "<output-file>" "<audit-dir-1>" "<audit-dir-2>" [...]
```

> The same dated-subfolder treatment applies — the script writes to `<parent-of-output-file>/reports/<YYYY-MM-DD>/<basename>` (e.g. pass `audits/combined-report.html` and it lands at `audits/reports/2026-05-21/combined-report.html`).

**Combined PowerPoint report** (comparative slide deck):
```bash
python3 "<project-root>/.claude/skills/ux-audit/scripts/generate_combined_pptx.py" \
  "<output-file>" "<audit-dir-1>" "<audit-dir-2>" [...]
```
...
2. **Save** all output to `audits/{YYYY-MM-DD}_{product-name}/`:
   ...
   └── reports/                         # Per-regeneration archive (auto-created by generate_html.py)
       ├── 2026-05-21/                  # One folder per generation date
       │   └── report.html              # Self-contained HTML (base64-embedded images)
       └── 2026-05-22/
           └── report.html              # Tomorrow's regeneration lives here
   ```
   ...
   │   └── 2026-05-21/
   │       └── combined-report.html               # Combined HTML (dated subfolder)
